 ...
 *@version    1.0
 */
public class ElementBean implements Comparable,Cloneable {
     ...
     *@since     2003年10月11日
     */
    public static Logger logger = Logger.getLogger("zt.build.form.config.ElementBean");
     ...
     *@since     2003年10月11日
     */
    public final static int DATATYPE_TIME = 5;
    /**
     *  Description of the Field
     *
     *@author    sun
    ...
    private platform.view.build.form.util.FieldFormat formatcls;
    ...
    public ElementBean(FieldBean field) {
        ...
        if (this.getComponetTp() == 0) {
            ...
            if (this.getDataType() == FieldBean.DATA_TYPE_TIME) {
                this.setComponetTp(ComponentType.TEXT_TYPE);
            }
        }
    }
    ...
    private boolean validateValue(Object value) {
        ...
        if (getDataType() == DATATYPE_ENUMERATION) {
            try {
                ...
                return EnumerationType.validate(getEnutpname(), (String)value);
            } catch (Exception ex) {
     ...
     */
    public platform.view.build.form.util.FieldFormat getFormatcls() {
        return formatcls;
    }
    ...
    public void setFormatcls(String formatcls, int type) {
        ...
        if (type == FormBean.LIST_TYPE || type == FormBean.QUERY_TYPE ) {
            try {
                platform.view.build.form.util.FieldFormat ff = (platform.view.build.form.util.FieldFormat) Class.forName(formatcls).newInstance();
                this.formatcls = ff;
            } catch (Exception ex) {
                this.formatcls = new platform.view.build.form.util.FieldFormat();
            }
        }
    }
