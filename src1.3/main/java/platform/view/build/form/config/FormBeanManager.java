...
import platform.view.build.form.util.datatype.DataType;
...
public class FormBeanManager {
    ...
    private static Logger logger = Logger.getLogger("zt.build.form.config.FormBeanManager");
    static {
        init();
    }
    ...
    private static void check(ElementBean eb, FieldBean fb) {
        if (fb == null) {
            ...
            if (eb.getComponetTp() == 0) {
                ...
                if (eb.getDataType() == FieldBean.DATA_TYPE_TIME) {
                    eb.setComponetTp(ComponentType.TEXT_TYPE);
                }
            }
        }
     ...
     *
     *@param  id
     *@return     zt.build.form.config.FormBean
