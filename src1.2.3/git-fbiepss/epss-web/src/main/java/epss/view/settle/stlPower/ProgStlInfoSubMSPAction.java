...
@ManagedBean
@ViewScoped
public class ProgStlInfoSubMSPAction {
    private static final Logger logger = LoggerFactory.getLogger(ProgStlInfoSubMSPAction.class);
    ...
    */
    ...
    public void onClickForMngAction(String strSubmitType){
        if(strSubmitType.equals("Add")){
            ...
            if ((EnumSubcttType.TYPE3.getCode()).equals(cttInfoTemp.getType())||
                ...
                progStlInfoShowQryQ.setStlPkid(progStlInfoShowDel.getStlPkid());
                progStlInfoShowQryQ.setStlType(EnumResType.RES_TYPE3.getCode());
    ...
    private void addRecordAction(ProgStlInfoShow progStlInfoShowPara){
        try {
            MessageUtil.addInfo(progStlInfoService.insertStlMAndItemRecordAction(progStlInfoShowPara));
        } catch (Exception e) {
    ...
    private void delRecordAction(ProgStlInfoShow progStlInfoShowPara){
        try {
            MessageUtil.addInfo(progStlInfoService.deleteStlMAndItemRecord(progStlInfoShowPara));
        } catch (Exception e) {
            logger.error("删除数据失败，", e);
            MessageUtil.addError(e.getMessage());
        }
    }
    private void deleteStlQAndItemRecordAction(ProgStlInfoShow progStlInfoShowPara) {
        try {
            progStlInfoService.deleteStlQAndItemRecord(progStlInfoShowPara);
        } catch (Exception e) {
            logger.error("删除数据失败，", e);
            MessageUtil.addError(e.getMessage());
        }
